...
contract StableFactory is Ownable {
  ...
  mapping(address => uint32) public rewards;
  ...
  uint256 public totalSZRClaimable;

  event AggregationRoundStarted(uint256 aggregationRoundId);
  event AggregationRoundCompleted(uint256 aggregationRoundId);
  event StableCreated(string country, string currency, address stableAddress, string[] productIds, uint8[] weightages);
  ...
  function createStable(
    ...
    Stable _stable = new Stable(_country, _currency, _productIds, _productWeightages);
  ...
  function updateAggregationSettings(string memory _priceAggregationMethod, uint8 _mininumPriceConfirmations)
    public
    onlyOwner
  {
    priceAggregationMethod = _priceAggregationMethod;
  ...
  function completeAggregation() public {
    ...
    for (uint8 i = 0; i < countries.length; i++) {
      require(Stable(childContracts[countries[i]]).priceUpdatedForAggRound() == true, "All countries not updated");
    }
    ...
    rewards[msg.sender] += aggregationRewardAmount;
  }
  ...
  function mintStable(uint32 _amount) public {
    ...
    require((mintableStable() + _amount) > currentSupply, "No collateral to mint");
    ...
    if (currentSupply + _amount > totalStablesRedeemable) {
      uint256 nonCollateralizedPercent = (currentSupply + _amount - totalStablesRedeemable) * 100 / totalStablesRedeemable;
      console.log(equivalentSZR, nonCollateralizedPercent);
  ...
  function claimSZR(uint32 _amount) public {
    require(SZRClaimableBySupplier(msg.sender) >= _amount, "Not enough balance");
  ...
  function SZRPriceInUSD() public view returns (uint256) {
    ...
    return 1000000 / (szrToken.totalSupply() / 2);
  }

  function stablePrice() public view returns (uint256) {
    return 100000 / globalPriceIndex(); // TODO: See if division by zero is possible
  }

  // Get amount of SZR claimable by the supplier
  function SZRClaimableBySupplier(address _account) public view returns (uint256) {
